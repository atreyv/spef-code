...
def cmask(index,radius,array):
    """Boolen array mask for image circular selection.
    Adapted from http://stackoverflow.com/questions/8647024/how-to-apply-a-disc-shaped-mask-to-a-numpy-array"""
    a,b = index
    nx,ny = array.shape
...
#Loading of Settings
def settings_load(filename, number_of_parameters):
    
    description = np.loadtxt(filename,skiprows=5)
    parameters = [[0,]*len(description) for i in range(number_of_parameters)]
    print description
    for i in range(0,number_of_parameters):
        parameters[i] = description[:,i]    
    return parameters

def folder_append(ID):
    rootDir   = '.'
    dir_names = np.array(['AAAAAAAAA',]*len(ID))
    for dirName, subdirList, fileList in os.walk(rootDir):
        #print('Found directory: %s' % dirName)
        j = re.split('(\d+)',dirName)
        if dirName != rootDir:
